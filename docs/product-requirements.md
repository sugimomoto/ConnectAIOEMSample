...
### Phase 4: AI アシスタント機能（次期開発）
7. MCP経由のAIチャットアシスタント
   - メインメニューから直接アクセス可能なチャットインターフェース
   - チャット内でカタログ（コネクション）を動的選択
   - ユーザー個別のClaude API Keyによる自然言語データ問い合わせ
   - バックエンド経由のMCPプロキシによるデータアクセス
   - MCPツール呼び出し履歴の可視化
...
- 🔲 AIチャットで自然言語からデータ取得までの流れをライブデモできる（Phase 4）
- 🔲 MCPツール呼び出し可視化により「AIがConnect AI経由でデータにアクセスしている」ことを説明できる（Phase 4）
...
### ストーリー6: Claude API Key の設定（Phase 4）
**As a** アプリのユーザー
...
### 6.7 AI アシスタント（Phase 4）

#### Claude API Key 管理
- **FR-AI-001**: ユーザーごとのAPI Key登録・管理
  - 設定画面でClaude API Keyを入力・更新・削除できる
  - API Keyは暗号化してDBに保存する
...
- **FR-AI-003**: チャット画面内のカタログ動的選択
  ...
  - カタログ未選択時でもチャットを開始でき、AIが必要なカタログを自律的に探索できる
  - ストリーミングレスポンス（SSE）で回答をリアルタイム表示する

#### バックエンドMCPプロキシ
- **FR-AI-004**: バックエンド経由のMCPツール呼び出し
  - フロントエンドはバックエンドAPIにチャットリクエストを送信
  - バックエンドがMCPクライアントとしてCData Connect AI MCPサーバーに接続
  - AIのツール呼び出し（getCatalogs, getSchemas, getTables, getColumns, queryData）をバックエンドが仲介・実行する
  - Connect AI の `accountId` によるテナント分離を維持する

#### 透明性・可視化
- **FR-AI-005**: MCPツール呼び出しの可視化
  - AIが実行したMCPツール呼び出しをアコーディオンUIで表示
  - ツール名・引数・レスポンスサマリーを確認可能
  - デモ時に「AIがConnect AI経由でデータにアクセスしている」ことを視覚的に説明できる
...
### Phase 4: AI アシスタント機能（次期開発）
**目標**: Connect AI OEM を採用したパートナーが自社製品に AI チャット機能を組み込めることを参照実装として示す

**コアフェーズ（最初に実装）**

| ID | 機能 | 状態 | 受け入れ条件 |
|----|------|------|------------|
| P4-1 | Claude API Key管理 | 🔲 | ユーザーがAPIキーを登録・保存でき、チャットリクエストに使われる |
| P4-2 | AIチャットUI（メニュー直接アクセス） | 🔲 | メニューからAIアシスタント画面に遷移できる |
| P4-3 | カタログ動的選択 | 🔲 | 画面内でカタログを切り替えられる |
| P4-4 | バックエンドMCPプロキシ | 🔲 | バックエンド経由でConnect AI MCPにアクセスし自然言語質問に回答できる |

**拡張フェーズ（コア完成後）**

| ID | 機能 | 状態 | 受け入れ条件 |
|----|------|------|------------|
| P4-5 | MCPツール呼び出し可視化 | 🔲 | AIの思考プロセス・データアクセス経路が画面で確認できる |
| P4-6 | 会話コンテキスト保持 | 🔲 | フォローアップ質問が自然に続けられる |
...
### Phase 4 追加分
- **AI モデル**: Claude API（claude-sonnet-4-6）— ユーザーごとに登録したAPI Keyを使用
- **MCPクライアント**: Python バックエンドで実装（Anthropic Python SDK）
- **MCP通信経路**: フロントエンド → Flask バックエンド → CData Connect AI MCPサーバー
- **ストリーミング**: Server-Sent Events（SSE）でチャット回答をリアルタイム表示
